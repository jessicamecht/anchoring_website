...
def main():
    ...
    for train_index, test_index in kf.split(data):
        ...
        acc_val, review_sessions, hidden_states = eval_anchor(data_test, anchor_lstm, step)
    ...
    generate_plot(review_sessions_lstm, f"./final_confidence_items")

    #torch.save(anchor_lstm.state_dict(), f'./state_dicts/anchor_lstm_items.pt')
...
def eval_anchor(data, anchor_lstm, step):
    ...
    for review_session in data:
            ...
            review_session = np.hstack((review_session, preds))
            review_sessions_lstm.append(np.array(review_session))

    #print("Correlation between reviewer decisions and hidden states: ", r)
    return num_correct/num_decisions, review_sessions_lstm, hidden_states#, correlation(review_sessions_lstm), review_sessions_lstm
