...
def main():         
    ...
    possible_idx_large = [1,2,3,4,5,6]
    possible_idx_mid = [13, 14]
    possible_idx_small = [7, 8, 9, 10, 11, 12]
    random_idx = random.choice(possible_idx_small)
    ...
    policy_net.load_state_dict(torch.load(f"./rl_anchoring/state_dicts/policy_model.pt", map_location=device))
    ...
    if button_placeholder_1.button("Yes, I'd like to read the book."):
        ...
        if np.array(ss.possible_next_instances_mask).sum() != 0:
            action_idx = get_next_action_dqn(lstm_input, ss.shown_instances, hidden_anchor_state, ss.possible_next_instances_mask, policy_net, anchor_lstm)
    ...
    if button_placeholder_2.button("No, I'd NOT like to read the book."):
        ...
        if np.array(ss.possible_next_instances_mask).sum() != 0:
            action_idx = get_next_action(lstm_input, ss.shown_instances, hidden_anchor_state, ss.possible_next_instances_mask, policy_net, anchor_lstm)
